...
public class ReleaseCorrectnessTests {
	...
	@Test
	void nullableCharsetOverloadClearsExplicitEncodingWithoutRemovingContextDefault() throws Exception {
		var context = SokletServletContext.builder().responseCharacterEncoding(StandardCharsets.UTF_16BE).build();
		var response = SokletHttpServletResponse.fromRawPath("/", context);
		response.setContentType("text/plain; charset=UTF-8");
		response.setCharacterEncoding((java.nio.charset.Charset) null);
		assertEquals("UTF-16BE", response.getCharacterEncoding());
		assertEquals("text/plain", response.getContentType());
		response.setCharacterEncoding(StandardCharsets.UTF_8);
		assertEquals("UTF-8", response.getCharacterEncoding());
		response.getWriter().write("é");
		response.setCharacterEncoding((java.nio.charset.Charset) null);
		assertEquals("UTF-8", response.getCharacterEncoding(), "Writer acquisition locks the encoding");
